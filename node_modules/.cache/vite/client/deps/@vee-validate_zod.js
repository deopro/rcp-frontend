import { O as ZodObject, f as ZodDefault, g as ZodFirstPartyTypeKind } from "./zod-CkBy9q83.js";
import { cleanupNonNestedPath, isNotNestedPath } from "./vee-validate.js";
//#region node_modules/@vee-validate/zod/dist/vee-validate-zod.mjs
/**
	...
	return {
		...
		cast(values) {
			try {
				...
				if (isObject(defaults) && isObject(values)) return merge(defaults, values);
				return values;
			}
		},
		describe(path) {
...
function getDefaults(schema) {
	...
	return Object.fromEntries(Object.entries(schema.shape).map(([key, value]) => {
		...
		if (value instanceof ZodObject) return [key, getDefaults(value)];
		return [key, void 0];
	}));
}
/**
...
function getSchemaForPath(path, schema) {
	...
	if (isNotNestedPath(path)) return schema.shape[cleanupNonNestedPath(path)];
	const paths = (path || "").split(/\.|\[(\d+)\]/).filter(Boolean);
	let currentSchema = schema;
	for (let i = 0; i <= paths.length; i++) {
		const p = paths[i];
		if (!p || !currentSchema) return currentSchema;
		if (isObjectSchema(currentSchema)) {
			currentSchema = currentSchema.shape[p] || null;
			continue;
		}
		if (isIndex(p) && isArraySchema(currentSchema)) currentSchema = currentSchema._def.type;
	}
	return null;
}
