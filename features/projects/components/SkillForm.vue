...
<template>
  <form class="space-y-4" @submit.prevent="onSubmit">
    <div class="space-y-1.5">
      <label class="text-sm font-medium" for="skill-name">{{ t('projects.fields.name') }}</label>
      <UiInput id="skill-name" v-model="form.name" required :disabled="!canEdit" />
    </div>
    <div class="space-y-1.5">
      <label class="text-sm font-medium" for="skill-category">{{ t('projects.fields.category') }}</label>
      <UiInput id="skill-category" v-model="form.category" :disabled="!canEdit" />
    </div>
    <div class="space-y-1.5">
      <label class="text-sm font-medium" for="skill-desc">{{ t('projects.fields.description') }}</label>
